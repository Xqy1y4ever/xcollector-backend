...
- **两种身份，不是两种权限等级**（实现见 `app/auth.py`）：
  ...
  | 令牌 | 谁持有 | 身份 | 能做什么 |
  |---|---|---|---|
  | `API_TOKEN` | **只有 bot** | `scope=service` | 共享层全部；按用户的接口**必须显式带 `user_id`** |
  | `UserToken`（`xc_…`） | 每个用户自己 | `scope=user` | 只能读写**他自己**的数据；`user_id` 参数被强制忽略 |
...
- **`user_id` 是 query 参数，不是 body 字段。** 按用户的接口（见下表）都接受
  `?user_id=usr_xxx`：
...
**都不带 `user_id`**（归属在这里没有意义），而且是**服务令牌专属**
（只有 bot 在写）。用户读原始消息是通过通知的读投影。

> 写前日志的落点：bot 收到消息后**第一件事**就是把它 POST 到这里，
> 然后才去下载附件、抽取。`state=pending` 同时充当"还没处理完"的恢复队列。
